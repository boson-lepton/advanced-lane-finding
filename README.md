...
A jupyter/iPython data science notebook was used and can be found on github [Full Project Repo](https://github.com/boson-lepton/advanced-lane-finding) - [Advanced Lane Finding Project Notebook](https://github.com/boson-lepton/advanced-lane-finding/blob/master/advanced-lane-finding.ipynb) (Note the interactive  ipywidgets are not functional on github). The project is written in python and utilises [numpy](http://www.numpy.org/) and [OpenCV](http://opencv.org/).
...
It involves reading a set of [warped chessboard images](https://github.com/boson-lepton/advanced-lane-finding/tree/master/camera_cal), converting them into grey scale images before using `cv2.findChessboardCorners()` to identify the corners as `imgpoints`.
![9x6 Chessboard Corners Detected](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/chessboard-corners-detected.png?raw=true)   
...
![Original and Undistorted image](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/camera_original_undistort.png?raw=true)
...
![test image distorted and undistorted](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/test_image_and_undistorted.png?raw=true)
...
![Sample Threshold Image](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/daytime-normal-threshold.png?raw=true)

`threshold` type parameter (`daytime-normal`, `daytime-bright`, `daytime-shadow`, `daytime-filter-pavement`).
...
![Daytime Normal with noise bright light & pavement change](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/daytime-normal-overexposed.png?raw=true)

![Daytime Normal with shadow](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/daytime-normal-shadow.png?raw=true)

Other samples [Daytime Bright](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/daytime-bright.png?raw=true), [Daytime Shadow](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/daytime-shadow.png?raw=true) and [Daytime Filter Pavement](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/daytime-filter-pavement.png?raw=true).

### Perspective transform - birds eye view
To be able to detect the road lines, the undistorted image is warped. The function `calc_warp_points` takes an image's height & width and then calculates the `src` and `dst` array of points. `perspective_transforms` takes them and returns two matrixes `M` and `Minv` for  `perspective_warp` and `perpective_unwarp` functions respectively. The following image, shows an undistorted image, with the src points drawn with the corresponding warped image (the goal here was straight lines) ![Distorted with bird's eye view](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/undistorted_with_birdseye.png?raw=true)
...
![topdown warped binary image](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/birdseye-combined-straighline-small.png?raw=true)
...
![Warped box seek and new polynomial fit](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/warped-box-seek-with-new-polyfit.png?raw=true)
...
![`lane.result_decorated`](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/result_with_lane_unwarped.jpg?raw=true)

![Lane.warped_decorated](https://github.com/boson-lepton/advanced-lane-finding/blob/master/output_images/birdseye-with-original-threshold-result-using-windowing.jpg?raw=true)

## Pipeline (Video)

Using [moviepy](http://zulko.github.io/moviepy/) to process the project video was simple. I also decorated the result with a frame count. The [Project Video Lane mp4 on GitHub](https://github.com/boson-lepton/advanced-lane-finding/blob/master/project_video_lane.mp4?raw=true)
